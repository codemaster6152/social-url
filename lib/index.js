...
const GITHUB_LINKS_PATTERN = /^\/([a-zA-Z0-9_.]{1,30})(\/.*)*$/i;
const STACKOVERFLOW_LINKS_PATTERN = /^\/users\/([a-zA-Z0-9_]{1,50}(?=(\/|$)))/i;
const MIXCLOUD_LINKS_PATTERN = /^\/([a-zA-Z0-9_]{3,255})(\/.*)*$/i;
const TWITCH_LINKS_PATTERN = /^\/([a-zA-Z0-9_]{1,50})(\/.*)*$/i;
...
const GITHUB_RESERVED_KEYWORDS = ['join'];
...
function parseGithub (url) {
  const match = GITHUB_LINKS_PATTERN.exec(url.pathname);
  let user = match ? match[1] : null;
...
function parseStackOverflow (url) {
  const match = STACKOVERFLOW_LINKS_PATTERN.exec(url.pathname);
  const user = match ? match[2] : null;
...
function parseMixcloud (url) {
  const match = MIXCLOUD_LINKS_PATTERN.exec(url.pathname);
  const user = match ? match[1] : null;
...
function parseTwitch (url) {
  const match = TWITCH_LINKS_PATTERN.exec(url.pathname);
  const user = match ? match[1] : null;
...
function parse (url) {
  ...
  if (domainName === 'google' && (subdomain === 'plus' || subdomain === 'www')) {
    ...
    return parseGithub(parsed);
  } else if (domainName === 'stackoverflow') {
    return parseStackOverflow(parsed);
  } else if (domainName === 'mixcloud') {
    return parseMixcloud(parsed);
  } else if (domainName === 'twitch') {
